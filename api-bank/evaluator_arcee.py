...
import os
from utils import ChatGPTWrapper, DavinciWrapper, GPT4Wrapper, AIONEWrapper, ArceeWrapper
import logging
...
logging.basicConfig(
    filename='evaluator_arcee.log', 
    filemode='w',    
    format='%(asctime)s - %(levelname)s - %(message)s',
    level=logging.INFO  
)
##LEVEL 1 API CALL OUTPUT FILE
# output_file = "output/level1/api_call_output_arcee.jsonl"
output_file = "output/level2/api_call_output_arcee.jsonl"
...
class Evaluator:
    ...
    def evaluate(self, sample_id, model_output):
        ...
        # Custom __repr__ method to provide more informative output
    def __repr__(self):
        return f"Evaluator(dataset_size={len(self.dataset)}, sample_ids={self.sample_ids[:5]}...)"
...
if __name__ == '__main__':
    ...
    chatgpt = ArceeWrapper(api_key='YOUR_API_KEY')
...
'''
    ...
    for file in tqdm(jsonl_files, desc='Processing files', ncols=100):
        ...
        if not samples:
            print(f"No samples created for file {file}")
        else:
        ...
        with open(output_file, "a") as jsonl_file:
            for sample_id in evaluator.get_all_sample_ids():
                ...
                if sample.ground_truth['role'] == 'API' and api_test_enabled:
                    ...
                    # print("API > MESSSAGES ", messages)
                    # print("sample.ground_truth['role']  ISSS ", sample.ground_truth['role'] )

                    response = chatgpt.call(messages)
                    if not response or 'choices' not in response:
                        logging.error(f"No valid response from model for file {file}, sample ID {sample_id}")
                    else:
                        # print(f"Model response: {response}")  # Debug: Check the model's raw response
                        model_output = response['choices'][0]['message']['content'] if isinstance(chatgpt, ArceeWrapper) else response['choices'][0]['text'].strip()
                    ...
                    final_input = f"{conversation_input}"

                    # print(f"API > response :{response}")
                    if isinstance(chatgpt, ArceeWrapper):
                    ...
                    record = {
                        "input": final_input, ###combine role:user, assistant into string
                        "file": file,
                        "id": sample_id,
                        "instruction":messages[0]['content'], ###role: system 
                        "raw_model_response": model_output,
                        "messages":messages,
                    }
